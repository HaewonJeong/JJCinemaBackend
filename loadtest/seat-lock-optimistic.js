...
const SEAT_CODE = 'A3';  // 지금 "선택 가능" 상태인 좌석이어야 함
...
const successCount = new Counter('optimistic_booking_success');
const conflictCount = new Counter('optimistic_booking_conflict_409');
const otherFailCount = new Counter('optimistic_booking_other_fail');
const successDuration = new Trend('optimistic_success_response_time');
const conflictDuration = new Trend('optimistic_conflict_response_time'); // 락 대기 후 실패까지 걸린 시간
...
export default function (data) {
  const res = http.post(
    `${BASE_URL}/api/bookings/optimistic`,
